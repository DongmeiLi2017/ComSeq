...
Combined_method <- function(RNAseqcount, label, alpha){
  ...
  group.con <- data.frame(cbind(c(rep("control", sum(label == 0)), rep("treatment", sum(label == 1))), c(rep("single-read", length(label)))))
  ...
  SAMSeq.test <- SAMseq(count.norm, label2, resp.type = "Two class unpaired", geneid = rownames(count.norm), genenames = rownames(count.norm), nperms = 100, nresamp = 20, fdr.output = alpha)
  ...
  mylist <- list(DESeq2.sig = genename[DESeq2.sig.gene], EBSeq.sig = genename[EBSeq.sig.gene], SAMSeq.sig = genename[SAMSeq.sig.gene],
                 NOISeq.sig = genename[NOISeq.sig.gene], Combined.sig = genename[common.sig], Combined.sig.table = common.select)
